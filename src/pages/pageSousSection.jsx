...
function PageSousSection() {
  ...
  const fetchNiveaux = async () => {
    ...
    try {
      let res = await axios.get(
        connection + "/lire-sous-section/" + locationSection
      );
      setNiveaux(res.data);
      setLoading(false);
    } catch (error) {
  ...
  const add = async (event) => {
    ...
    try {
      let res = await axios.post(
        connection + "/ajouter-sous-section",
        sectionFormData
      );
    } catch (error) {
      setServerError(true);
    ...
    niveaux.forEach((niveau) => {
      if (niveau.idNiveau) {
        ...
        if (niveau.nombreNiveaux == 1) {
          ...
          nombre = niveau.nombreNiveaux + " cellules";
        } else {
          nombre = "Aucune cellule";
        }
        listNiveaux.push(
          <div className="niveau" key={niveau.idNiveau}>
            <div
              onClick={() => {
                navigate(`/cellule/${locationInterFederation}-${locationFederation}-${locationSection}-${niveau.idNiveau}`);
              }}
              style={{
                padding: "0.5em",
                borderRadius: "13px",
  ...
  const goBack = () => {
    navigate("/section/"+locationInterFederation+'-'+locationFederation+'-'+locationSection);
  };
    ...
    <>
      ...
      <div className="header-title">
            ...
            {listNiveaux.length > 1 && (
              ...
              >
                {listNiveaux.length} sous sections
              </h4>
            )}
          </div>
        <div
...
export default PageSousSection;
