  ...
  return (
    <div className="etape-container etape-sexe slide-in-right">
      ...
      <div className="sexe-selection">
        <button type="button" onClick={() => handleSexeChange('M')} className="sexe-button">
          <img src="../img/male.png" alt="Homme" className="sexe-image" /> <p>Homme</p>
        </button>
        <button type="button" onClick={() => handleSexeChange('F')} className="sexe-button">
  ...
  return (
    <div className="etape-container etape-presentation">
      ...
      <div className="form-group">
        ...
        <input type="text" id="postnom" name="postnom" value={formData.postnom || ''} onChange={handleChange} />
      </div>
      <div className="form-group">
  ...
  return (
    <div className="etape-container etape-adresse">
      ...
      <div className="form-group">
        ...
        <input type="text" id="commune" name="commune" value={formData.commune || ''} onChange={handleChange} required />
      </div>
      <div className="form-group">
        <label htmlFor="avenue">Avenue:</label>
        <input type="text" id="avenue" name="avenue" value={formData.avenue || ''} onChange={handleChange} required />
      </div>
      <div className="form-group">
...
const EtapeOrigine = ({ nextStep, prevStep, updateFormData, formData }) => {
  ...
  return (
    <div className="etape-container etape-origine">
      <h3>Où et quand etes vous né ?</h3>
      <div className="form-group">
        <label htmlFor="lieuNaissance">Lieu de Naissance:</label>
        <input type="text" id="lieuNaissance" name="lieuNaissance"  value={formData.lieuNaissance || ''} onChange={handleChange} required />
      </div>
      <div className="form-group">
        <label htmlFor="dateNaissance">Date de Naissance:</label>
        <input type="date" id="dateNaissance" name="dateNaissance" min="1900-01-01" max="2010-12-31" value={formData.dateNaissance || ''} onChange={handleChange} required />
      </div>
      <div className="form-group">
        <label htmlFor="provinceOrigine">Province d'Origine:</label>
        <input type="text" id="provinceOrigine" name="provinceOrigine" value={formData.provinceOrigine || ''} onChange={handleChange} required />
      </div>
    </div>
  );
};
  ...
  return (
    <div className="etape-container etape-termes">
      ...
      <div className="termes-texte">
        <p>Moi {formData.nom} {formData.postnom} {formData.prenom}, je m'engage à devenir membre de l'AADC, à respecter et à faire respecter les status, le règlement intérieur et résolutions des organes du parti.</p>
      </div>
      <div className="form-group">
        <label className="checkbox-label" >
          <input type="checkbox" style={{marginLeft:"auto"}} name="accepte" checked={formData.accepte || false} onChange={handleAcceptChange} />
          J'accepte.
        </label>
      </div>
    </div>
...
const Recapitulatif = ({ formData, prevStep, handleSubmit }) => {
  ...
  return (
    <div className="etape-container recapitulatif">
      ...
      <div className="recap-item"><strong>Nom:</strong> {formData.nom}</div>
      {formData.postnom && <div className="recap-item"><strong>Postnom:</strong> {formData.postnom}</div>}
      <div className="recap-item"><strong>Prénom:</strong> {formData.prenom}</div>
      {formData.sexe && <div className="recap-item"><strong>Sexe:</strong> {formData.sexe}</div>}
      {formData.province && <div className="recap-item"><strong>Province:</strong> {formData.province}</div>}
      {formData.commune && <div className="recap-item"><strong>Commune:</strong> {formData.commune}</div>}
      {formData.avenue && <div className="recap-item"><strong>Avenue:</strong> {formData.avenue}</div>}
      {formData.numero && <div className="recap-item"><strong>Numéro:</strong> {formData.numero}</div>}
      {formData.lieuNaissance && <div className="recap-item"><strong>Lieu de Naissance:</strong> {formData.lieuNaissance}</div>}
      {formData.dateNaissance && <div className="recap-item"><strong>Date de Naissance:</strong> {formData.dateNaissance}</div>}
      {formData.provinceOrigine && <div className="recap-item"><strong>Province d'Origine:</strong> {formData.provinceOrigine}</div>}
      {formData.etatCivil && <div className="recap-item"><strong>État Civil:</strong> {formData.etatCivil}</div>}
      {formData.profession && <div className="recap-item"><strong>Profession:</strong> {formData.profession}</div>}
      {formData.telephone && <div className="recap-item"><strong>Téléphone:</strong> {formData.telephone}</div>}
      {formData.email && <div className="recap-item"><strong>Adresse Email:</strong> {formData.email}</div>}
...
const AdhesionForm = () => {
  ...
  const nextStep = () => {
    if((currentStep == 2)&& (!formData.nom || !formData.prenom || ! formData.postnom)){alert(`S'il vous plait, ${appelation} presentez vous !`);return}
    if((currentStep == 3) && (!formData.province || !formData.commune || ! formData.avenue || ! formData.numero)){alert(`Entrez votre adresse complet, ${appelation} ${nom}`);return}
    if((currentStep == 4)&& (!formData.lieuNaissance || !formData.dateNaissance || ! formData.provinceOrigine)){
      if(!formData.dateNaissance && formData.lieuNaissance && formData.provinceOrigine){
        alert(`Vous avez oubliez de donner la date de naissance, ${appelation} ${nom}`)
      }else{
        alert(appelation + " " + nom + ", veuillez remplir tous les champs, s'il vous plait")
      }
      return}
    if((currentStep == 5) && (!formData.etatCivil || !formData.profession)){alert("Parlez de votre vie social, " + appelation + " " + nom);return}
    if((currentStep == 6) && (!formData.telephone || !formData.email)){alert(`Donnez tous vos contacts, ${appelation} ${nom}`);return}
    if(currentStep == 7 && !formData.photo){alert(`Nous avons besoin de savoir votre visage, ${appelation} ${nom}`);return}
    if(currentStep == 8 && !formData.typeMembre){alert(`Veuillez choisir le type ${appelation} ${nom}`);return}
    if(currentStep == 9 && !formData.accepte){alert(`${appelation} ${nom}, vous devez d'abord accepter avant de devenir membre.`);return}
...
console.log(formData)
    ...
    try {
      let response = await axios.post(connection + "/adhesion-en-ligne", formData);
