...
function PageInterfederation() {
  ...
  const fetchNiveaux = async () => {
    setLoading(true);
    try {
      let res = await axios.get(connection + "/lire-interfederation");
      setNiveaux(res.data);
      setLoading(false);
    } catch (error) {
      setServerError(true);
  ...
  const add = async (event) => {
    ...
    try {
      let res = await axios.post(
        connection + "/ajouter-interfederation",
        FederationFormData
      );
    } catch (error) {
      setServerError(true);
    ...
    niveaux.forEach((niveau) => {
      if (niveau.idNiveau) {
        ...
        if (niveau.nombreNiveaux == 1) {
          ...
          nombre = niveau.nombreNiveaux + " federations";
        } else {
    ...
    <>
      ...
      <div className="header-title">
            ...
            {listNiveaux.length > 1 && (
              ...
              >
                {listNiveaux.length} interfederations
              </h4>
            )}
          </div>
        <div
...
export default PageInterfederation;
